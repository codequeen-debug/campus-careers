...
export default function AdminDashboard() {
  ...
  async function toggleStatus(userId) {
    ...
    const nextStatus = user.status === "Pending"
      ? "Active"
    ...
    <div style={{ background: "#fff5f8", minHeight: "calc(100vh - 64px)", padding: "40px 2rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: "#475569", fontSize: 13, marginTop: 14, maxWidth: 740 }}>
            Recruiter accounts are reviewed automatically after sign up. Approve pending recruiter profiles here before they can access the recruiter dashboard and job posting tools.
          </p>
        </div>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u.id} style={{ borderBottom: "1px solid #fff5f8" }}>
                        ...
                        <td style={{ padding: 12 }}>
                          <span style={{
                            color: u.status === "Suspended" ? "#d4818d"
