...
export default function JobDetail() {
  ...
  useEffect(() => {
    async function fetchJobAndCheckApplication() {
      try {
        ...
        if (currentUser) {
          ...
          const appsRes = await fetch(`${API_BASE}/api/applications/by-applicant/${userId}`);
          if (appsRes.ok) {
            const apps = await appsRes.json();
            const hasApplied = apps.some(app => app.jobId === id);
  ...
  async function handleApply(e) {
    ...
    if (!resumeFile) {
      setError("Please upload a resume/CV file.");
      return;
    }
    setSubmitting(true);
    setError("");
    try {
      ...
      formData.append("jobId", job._id || id);
      formData.append("jobTitle", job.title);
      formData.append("institution", job.institution);
      formData.append("applicantId", userId);
      formData.append("applicantName", userData?.name || "");
          ...
          <div style={{ background: "#ffffff", borderRadius: 16, padding: 36, maxWidth: 520, width: "100%", border: "1px solid #ffebf1", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.05)" }}>
            {submitSuccess ? (
              <div style={{ textAlign: "center" }}>
                ...
                <h2 style={{ color: "#057a55", fontSize: 20, fontWeight: 700, margin: "0 0 12px", fontFamily: "Georgia, serif" }}>Application Submitted!</h2>
                <p style={{ color: "#64748b", fontSize: 14, margin: 0 }}>Thank you for applying to {job.title}. The recruiter will review your application and be in touch soon.</p>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={handleApply}>
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ color: "#64748b", fontSize: 13, display: "block", marginBottom: 6 }}>Resume/CV *</label>
                    <div style={{ ...inputStyle, padding: "12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <label style={{ flex: 1, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ color: resumeFile ? "#057a55" : "#94a3b8", fontSize: 14 }}>
                    ...
                    <p style={{ color: "#94a3b8", fontSize: 11, margin: "4px 0 0" }}>PDF, DOC, DOCX, or TXT (max 5MB)</p>
                  </div>
                  <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <label style={{ color: "#64748b", fontSize: 13, display: "block", marginBottom: 6 }}>References (optional)</label>
                    <textarea
                      placeholder="List your professional references with contact information (name, title, email, phone)"
                      value={references}
